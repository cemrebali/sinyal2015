[nota,b,c]=textread('notalar.txt','%s %s %s'); %Notalar.txt dosyasý okutuldu. Okunan her sütun bir deðere atandý. 
...
a=zeros(1,length(notalar)); %notalar matrisi boyutlarýnda sýfýrdan olusan bir dizi olusturuldu
durak=zeros(1,ds); %durma süresi boyutunda sýfýrdan olusan bir dizi olusturuldu
oktav=str2num(cell2mat(b)); %dosyadan cell olarak okunan degiskenler double degere d?önüþtürüldü.
   for i=1:(length(nota)) %nota dizisi boyu kadar döngü oluturuldu
       ...
       frekans((i),1)=frek(nota((i),1),oktav((i),1)); %geçen hafta oluþturulan frek fonksiyonu çaðýrýldý
       [x,t]=note(frekans((i),1),vurus((i),1)); %geçen hafta oluþturduðumuz note fonksiyonu çaðrýldý
